...
<script>
    ...
    import {student_examine_result_edit,student_examine_result_info} from '@/api/getDatastudent_examine_result'
    export default {
        data(){
            return {
                id:0,
        ...
        methods: {
            ...
            submit: function () {
                ...
                this.$confirm('确认无误, 是否继续?', '提示', {
                    ...
                    this.loading = true;
                    student_examine_result_edit({id:this.id,name:this.name}).then(function (res) {
                        if (res.code == this.$store.state.constant.status_success) {
                            this.$message({
                                message: res.msg,
                                type: 'success'
                            });
                            this.$router.push({path:'student_examine_result',query:{}});
                        } else {
                            this.$message({
                                message: res.msg,
                                type: 'warning'
