    ...
    <div class="fillcontain">
        ...
        <div class="table_container" style="padding:20px">
            ...
            <div class="search_item" style="margin-bottom: 20px;">
                <span class="pre_info" style="font-size: 14px;margin-left: 30px;width: 150px;"><i style="color:red;">*</i>标题</span>
                <el-input clearable placeholder="标题" v-model="title" style="width: 350px">

                </el-input>
            </div>

            <p class="search_item" style="font-size: 12px;display: inline-block;vertical-align: top">   <span class="pre_info" style="font-size: 14px;margin-left: 30px;width: 150px;"><i style="color:red;">*</i>入口图</span></p>
            <el-upload
...
<script>
    ...
    import {sign_course_info,sign_stage_info,get_course_list,sign_course_edit,sign_stage_edit,get_class_list,examination_all_list} from '@/api/getDataEarth'
    export default {
        data(){
            return {
                id:0,
        ...
        methods: {
            ...
            get_info() {
                return new Promise(function(resolve,reject) {
                    sign_stage_info({id: this.id}).then(function (res) {
                        if (res.code == this.$store.state.constant.status_success) {
            ...
            submit: function () {
                ...
                var error_msg = [];
                if (!this.title) {
                    error_msg.push('请输入标题');
                }
                if (!this.img) {
                    error_msg.push('请上传入口图');
                }
                ...
                this.$confirm('共计'+this.courses_data.length+'个课程报名信息即将发布, 是否继续?', '提示', {
                    ...
                    sign_course_edit({
                        ...
                        if (res.code == this.$store.state.constant.status_success) {
                            ...
                            this.$router.push({path:'sign_stage',query:{}});
                        } else {
                            this.$message({
                                message: res.msg,
                                type: 'warning'
