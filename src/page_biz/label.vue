        ...
        <div class="table_container">
            <el-table
                ...
                <el-table-column label="操作" width="300">
                    <template slot-scope="scope">
                        <el-button size="mini" @click="goto_edit_label(scope.row.id)">编辑</el-button>
        ...
        methods: {
            ...
            verify(scope, status) {
                ...
                this.$confirm('确认此操作?', '提示', {
                    ...
                    label_verify({id:item.id,status:status}).then(function(res){
                        if (res.code == this.$store.state.constant.status_success) {
                            item.status = status;
                            this.$message({
                                type: 'success',
            ...
            del(item, index) {
                ...
                this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
                    ...
                    label_del({id:item.id}).then(function(res){
                        if (res.code == this.$store.state.constant.status_success) {
